<?php
namespace PUREHEARTS\Includes\Classes;
/**
 * Common functions
 */
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => purehearts_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => purehearts_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => purehearts_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => purehearts_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => purehearts_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => purehearts_meta( 'banner_elementor_template', $page_id ),
				'banner'            => purehearts_set( purehearts_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => purehearts_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => purehearts_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : purehearts_the_title( $template ),
				'banner'        => purehearts_set( $bg, 'url' ),
